...
| 11 | Monitoring dashboards + alerts live (§7) | ⚠️ | **Alerts: three SLOs proven end to end** by deliberate drill, each watched to open an issue *and deliver mail to a human* — NFR-06 crash-free sessions, NFR-14 availability (via `SVC_health`), and NFR-07 crash-free users. That is §8.4's standard rather than "configured". ⛔ **Dashboards: absent** — §7.2's dashboards need ADR-025's `analytics_event` rollup worker, which is unbuilt. ✅ **The two open metric-monitor issues are CLEARED (2026-08-08)** — both crash-free monitors were recreated, which is the only lever that works, and `PANCHANGPAL-MOBILE-4`/`-5` went with them. Nothing is currently suppressing the next alert. ⚠️ **Not permanent:** any further drill re-opens one, so re-check immediately before launch rather than trusting the date. ✅ **And both are drill-proven as well as clean** (2026-08-08): each has been watched delivering **its own** email to a human, and each alert action was confirmed to target an explicit **Member** — the precise field drill 1 got wrong. All three properties hold at once (proven · correctly addressed · no open issue), which is the strongest state this project has recorded for alerting. ⚠️ Re-check before launch, since a later drill re-opens an issue; `SLO_ALERTS.md` §9 records how the two properties are reached together. **Owner: ENG** |
...
| 2. Beta cohort / canary | ⛔ No | Step 1, plus item 11's dashboards to watch it with. (The metric-monitor issues that stood here are ✅ **cleared as of 2026-08-08** — re-check before launch, since a drill re-opens them.) |
