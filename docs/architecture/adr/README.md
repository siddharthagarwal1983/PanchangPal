...
| [ADR-035](ADR-035-Preferences-Sync-Conflict-Rule.md) | Preferences Sync Conflict Rule | Proposed | 2026-08-02 | TDD Part 2 §6.6 defines rules for three syncable kinds and omits `preferences`, which became a fourth in PR #86 and shipped on an adopted precedent. Settles the per-column merge on engineering grounds; **refers whose edit wins across devices to Product**. ⚠️ Records that the documented LWW rule is **not implemented** — `resolvePreferences` is called with `null` and the upsert is unguarded, so actual behaviour is last-drain-wins. |

---

*35 ADRs (ADR-033, ADR-034 and ADR-035 are **Proposed** — the canonical panchang engine, the account-deletion audit record, and the preferences sync conflict rule; all others `Accepted` for v1). Two are marked `[TECHNICAL IMPROVEMENT]` in their source (ADR-009, ADR-010; ADR-021 improves an implied approach). ADR-011/012/013 resolve TDD follow-ups F-20/F-18/F-19; ADR-033 realizes ADR-010's engine; ADR-034 is bounded by ADR-031's minimization rule. See `ADR_TEMPLATE.md` for the official template and authoring guidelines. Companion docs for ADR-033: `docs/architecture/canonical-panchang-engine/`.*
