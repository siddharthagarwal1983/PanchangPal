...
| [ADR-034](ADR-034-Account-Deletion-Audit-Record.md) | Account-Deletion Audit Record | Accepted | 2026-08-02 | Resolves the TDD Part 5 §5.1 vs Part 2 §3.15 conflict: the deletion *request* and the deletion *audit* have opposite lifetimes and cannot be one row. Separates them; retires the unwritable `executed_at`. **Ratified 2026-08-02 as Alternative C — a one-way sha256 digest of the subject**, so an operator can verify a specific claim without the table being a readable roster of people who asked to be forgotten. Implemented and proven by 6 pgTAP assertions. |
| [ADR-035](ADR-035-Preferences-Sync-Conflict-Rule.md) | Preferences Sync Conflict Rule | Accepted | 2026-08-02 | TDD Part 2 §6.6 defines rules for three syncable kinds and omits `preferences`, which became a fourth in PR #86 and shipped on an adopted precedent. Settles the per-column merge on engineering grounds; **Product ratified last-writer-wins on `local_ts`** the same day. ⚠️ Drafting it found the documented rule was **not implemented** — `resolvePreferences` was called with `null` and the upsert unguarded, making actual behaviour last-drain-wins with `updated_at` free to move backwards; **fixed in the same change**, with sequencing tests that fail against the original defect. |

---

*35 ADRs (**ADR-033 is the only one still Proposed** — the canonical panchang engine; ADR-034 and ADR-035 were both ratified on 2026-08-02; all others `Accepted` for v1). Two are marked `[TECHNICAL IMPROVEMENT]` in their source (ADR-009, ADR-010; ADR-021 improves an implied approach). ADR-011/012/013 resolve TDD follow-ups F-20/F-18/F-19; ADR-033 realizes ADR-010's engine; ADR-034 is bounded by ADR-031's minimization rule. See `ADR_TEMPLATE.md` for the official template and authoring guidelines. Companion docs for ADR-033: `docs/architecture/canonical-panchang-engine/`.*
