...
| [ADR-034](ADR-034-Account-Deletion-Audit-Record.md) | Account-Deletion Audit Record | Proposed | 2026-07-28 | Resolves the TDD Part 5 §5.1 vs Part 2 §3.15 conflict: the deletion *request* and the deletion *audit* have opposite lifetimes and cannot be one row. Separates them; retires the unwritable `executed_at`. **What identifies the subject of a completed erasure is owed ratification** (Security/Privacy + Legal). |
...
*35 ADRs (ADR-033 and ADR-034 remain **Proposed** — the canonical panchang engine and the account-deletion audit record; ADR-035 was ratified the day it was raised; all others `Accepted` for v1). Two are marked `[TECHNICAL IMPROVEMENT]` in their source (ADR-009, ADR-010; ADR-021 improves an implied approach). ADR-011/012/013 resolve TDD follow-ups F-20/F-18/F-19; ADR-033 realizes ADR-010's engine; ADR-034 is bounded by ADR-031's minimization rule. See `ADR_TEMPLATE.md` for the official template and authoring guidelines. Companion docs for ADR-033: `docs/architecture/canonical-panchang-engine/`.*
