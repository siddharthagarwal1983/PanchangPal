...
*34 ADRs (ADR-033 and ADR-034 are **Proposed** — the canonical panchang engine, and the account-deletion audit record; all others `Accepted` for v1). Two are marked `[TECHNICAL IMPROVEMENT]` in their source (ADR-009, ADR-010; ADR-021 improves an implied approach). ADR-011/012/013 resolve TDD follow-ups F-20/F-18/F-19; ADR-033 realizes ADR-010's engine; ADR-034 is bounded by ADR-031's minimization rule. See `ADR_TEMPLATE.md` for the official template and authoring guidelines. Companion docs for ADR-033: `docs/architecture/canonical-panchang-engine/`.*
