...
describe('useLocalDate', () => {
  ...
  it('returns null until the zone is known, rather than guessing a day', async () => {
    jest.setSystemTime(new Date('2026-07-22T21:00:00Z'));
    const { result } = await renderHook(() => useLocalDate());
    expect(result.current).toBeNull();
  });

  it('names the local day, not the UTC day', async () => {
  ...
  it('gives the same instant a different day in a different zone', async () => {
  ...
  it('rolls over on its own while the app stays open', async () => {
  ...
  it('recomputes on foreground, because timers do not run reliably in the background', async () => {
  ...
  it('follows a zone change, so correcting your location fixes the day immediately', async () => {
    jest.setSystemTime(new Date('2026-07-22T21:00:00Z'));
    setZone('America/New_York');
